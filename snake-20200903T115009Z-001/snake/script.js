...
function Game() {
    ...
    this._createSnake = function () {
        ...
        let startX = this.FIELD_SIZE_X / 2;
        let startY = this.FIELD_SIZE_Y / 2;

        for (let i = 0; i < this.SNAKE_SIZE && startY + i < this.FIELD_SIZE_Y; i++) {
            let snakeCell = getCell(startX, startY + i);
    ...
    this._wallsCreator = function (){
        let x = getRandom(0, game.FIELD_SIZE_X - 1);
        let y = getRandom(0, game.FIELD_SIZE_Y - 1);
        let wallCell = getCell (x,y)
        if (!game.snake.isSnakeCell(wallCell) && wallCell != game.foodCell) {
            wallCell.classList.add("wall-cell")
            wallCell.dataset.livetime = getRandom (game.SNAKE_SPEED * 3, game.FIELD_SIZE_X * game.SNAKE_SPEED )
